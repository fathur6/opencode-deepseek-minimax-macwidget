...
**Domain:** macOS menu bar LLM usage widget — OpenAI/ChatGPT Plus quota-reset timeline (adding next-reset date+time, usage bar, and 168h elapsed-hours marker to an existing SwiftUI menu bar app)
**Researched:** 2026-08-09
**Confidence:** HIGH for endpoint/auth/schema (verified across 3 independent implementations); MEDIUM for undocumented-API stability; LOW for endpoint persistence

## Executive Position

There is **no public, documented API** for ChatGPT Plus subscription quota. The ChatGPT consumer product (chatgpt.com) is separate from the OpenAI API platform (api.openai.com); a Plus subscription does not include API quota, and every documented OpenAI usage endpoint returns API *spend in dollars*, never Plus message quota or reset times. The only working approach — used by every tool in this niche, including this repo's own `feature/openai-quota` branch — is the **undocumented internal ChatGPT backend endpoint `GET https://chatgpt.com/backend-api/wham/usage`**, authenticated with the OAuth access token that the **Codex CLI** writes to `~/.codex/auth.json`. The response is server-authoritative: it contains the **reset timestamp (`reset_at`, unix seconds)** for both the 5-hour rolling window (`primary_window`) and the **7-day / 168-hour weekly window (`secondary_window`)** — the 168h cycle in this project's model maps exactly to `secondary_window`. The widget's remaining-% and elapsed-hours marker must be computed from these server timestamps, not derived from local observation.
...
**Response schema (verified, both reference apps + prior branch agree):**
...
```json
{
  ...
  "rate_limit": {
    "limit_reached": false,
    "primary_window":   { "used_percent": 12, "reset_at": 1787000000 },
    "secondary_window": { "used_percent": 1,  "reset_at": 1787534880 }
  },
...
**Exact fetch recipe (matches prior branch + Python tool):**
...
if let accountID, !accountID.isEmpty {
    request.setValue(accountID, forHTTPHeaderField: "ChatGPT-Account-ID") // optional, for multi-account
}
// Python reference also sends: User-Agent: codex_cli/0, Originator: codex_cli
```

**Decoding structs (keep BOTH windows — the prior branch only kept one and must be extended):**

```swift
struct WhamUsageResponse: Decodable {
    let planType: String?
    let rateLimit: RateLimit?
    enum CodingKeys: String, CodingKey { case planType = "plan_type", rateLimit = "rate_limit" }
}
...
# 4. Build:
cd opencode-widget && xcodegen generate
...
**If the user has signed in to Codex at least once (typical for an OpenCode user):**
...
**If `~/.codex/auth.json` is missing or the endpoint returns 401:**
...
**If the user needs multiple Plus accounts / workspaces:**
...
**For the elapsed-hours marker (168h cycle):**
...
**For the "reset date AND time" line:**
...
---
*Stack research for: OpenAI/ChatGPT Plus quota-reset timeline in the OpenCode macOS usage widget*
*Researched: 2026-08-09*
